        ...
        const payload = {
            ...
            roles: '', // Se ignora, backend asigna 'user'
            parentesco: {
        ...
        ApiService.post('/users', payload)
            .then(() => {
                ...
                .then((response) => {
                    ...
                    localStorage.setItem('userRole', roles); // ⚠️ roles es string 'user'

                    // Redirigir al flujo de usuario normal
                    navigate('/app/patient-intro');
                })
                .catch(() => {
                    setErrorMessage('Usuario registrado, pero error al iniciar sesión automáticamente.');
                    navigate('/login');
                });
            })
            .catch(error => {
