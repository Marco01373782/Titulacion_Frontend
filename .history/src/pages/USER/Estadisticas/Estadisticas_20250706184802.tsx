    ...
    if (userId) {
        getSesionesUsuario(userId).then(async res => {
            const sesionesConPromedio = await Promise.all(res.data.map(async (s: any) => {
    const sessionId = s.sesionId || s.sesion?.id;
    if (!sessionId) return { ...s, result: 0 }; // evita el error 400

    try {
        const avgRes = await getAverageForSessionAndUser(sessionId, parseInt(userId));
        return { ...s, result: avgRes.data };
    } catch {
        return { ...s, result: 0 }; // fallback en caso de error
    }
}));
