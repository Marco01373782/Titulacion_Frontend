...
import { Patient } from '../../../models/Patient';  // suponiendo que ya lo tienes separado
...
const PatientEdit = () => {
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{
      width: '100%', maxWidth: 600, mx: 'auto', p: 3, bgcolor: theme.palette.background.paper,
      borderRadius: 2, boxShadow: 3
    }} noValidate>
      <Typography variant="h4" mb={3} textAlign="center" color="primary">Editar Paciente</Typography>
      <Box sx={{ textAlign: 'center', mb: 3 }}>
        ...
        <label htmlFor="upload-photo" style={{ cursor: 'pointer' }}>
          <Avatar src={imagePreview || ''} sx={{
            width: isMobile ? 140 : 180, height: isMobile ? 140 : 180, mx: 'auto',
            bgcolor: theme.palette.grey[300], mb: 1
          }}>
            {!imagePreview && 'Subir Foto'}
          </Avatar>
        </label>
        {formErrors.imageFile && <FormHelperText error>{formErrors.imageFile}</FormHelperText>}
      </Box>

      <TextField label="Primer Nombre *" name="firstname" fullWidth value={form.firstname} onChange={handleChange}
        error={!!formErrors.firstname} helperText={formErrors.firstname} sx={{ mb: 2 }} />
      <TextField label="Segundo Nombre" name="secondname" fullWidth value={form.secondname} onChange={handleChange} sx={{ mb: 2 }} />
      <TextField label="Apellido *" name="surname" fullWidth value={form.surname} onChange={handleChange}
        error={!!formErrors.surname} helperText={formErrors.surname} sx={{ mb: 2 }} />
      <TextField label="Edad *" name="age" type="number" inputProps={{ min: 1 }} fullWidth value={form.age}
        onChange={handleChange} error={!!formErrors.age} helperText={formErrors.age} sx={{ mb: 2 }} />

      <FormControl fullWidth error={!!formErrors.gender} sx={{ mb: 3 }} required>
        <InputLabel id="gender-label">Género *</InputLabel>
        <Select labelId="gender-label" label="Género *" name="gender" value={form.gender} onChange={handleSelectChange}>
      ...
      <Button type="submit" variant="contained" color="primary" fullWidth disabled={saving} size="large">
