    ...
    const handleClickSesion = (sesionId: number) => {
        window.location.href = `/user/Sesionesgrupales/${sesionId}`;
    };
    ...
    return (
        ...
        <div className="group-grid-item" onClick={() => handleClickSesion(sesion.id)} style={{ backgroundColor: 'lightblue' }}>
  <h3>{sesiones.nombre || 'Sin nombre'}</h3>
  <p>{sesion.descripcion || 'Sin descripción'}</p>
</div>
