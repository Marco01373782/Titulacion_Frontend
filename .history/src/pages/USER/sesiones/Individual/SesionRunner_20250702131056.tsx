...
const SesionRunner: React.FC = () => {
    ...
    if (!ActivityComponent) return (
        <Box sx={{ width: '100%', height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', background-color: linear-gradient(135deg, #e0f7fa, #fef6ff); }}>
            <Typography variant="h5">No se encontró el componente para: {resourceUrl}</Typography>
        </Box>
    );

    return (
        <Box sx={{ width: '100%', height: '89vh', bgcolor: '#f4f5fa', p: 2, position: 'relative' }}>
            <Button
                variant="outlined"
                color="secondary"
                onClick={toggleMusic}
