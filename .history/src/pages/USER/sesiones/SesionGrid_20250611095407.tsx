    ...
    loadSessions();
      ...
      const handleSesionClick = (sesionUsuario: SesionUsuario) => {
  const currentUserId = Number(localStorage.getItem("userId"));
  
  // ✅ Solo guarda si la sesión pertenece al usuario actual
  if (sesionUsuario.user.id === currentUserId) {
    localStorage.setItem("selectedSesionUsuarioId", sesionUsuario.id.toString());
    console.log("✅ SesionUsuarioId guardado:", sesionUsuario.id);
    setModalSesion(sesionUsuario);
  } else {
    console.warn("❌ Sesión seleccionada no pertenece al usuario actual.");
  }
};
