...
const SesionRunner: React.FC = () => {
    ...
    if (activities.length > 0 && currentIndex < activities.length) {
        ...
        // Solo al iniciar la primera actividad, registramos el inicio de sesión
        if (currentIndex === 0 && !sesionStartTime) {
            const now = new Date();
            setSesionStartTime(now);

            // Opcional: actualizar el status de la sesión en backend
            updateSesionUsuarioStatus(sesionUsuarioId, 'EN_PROGRESO')
                .then(() => console.log('🟢 Sesión marcada como EN_PROGRESO'))
                .catch((e) => console.error('❌ Error al cambiar estado a EN_PROGRESO', e));
        }
    ...
    const handleFinishActivity = async (score: number | null) => {
        ...
        try {
            ...
            console.log('✅ resultado enviado enviado:', payload.result); // <-- este te confirma que se envió
    ...
    if (currentIndex < activities.length - 1) {
        ...
        const totalDurationFromStart =
            sesionStartTime ? Math.floor((new Date().getTime() - sesionStartTime.getTime()) / 1000) : 0;
        ...
        const payload = {
            ...
            sessionDurationSeconds: totalDurationFromStart,
