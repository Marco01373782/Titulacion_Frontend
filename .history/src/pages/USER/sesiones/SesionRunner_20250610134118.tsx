        ...
        const handleFinishActivity = async (score: number) => {
            ...
            const payload = {
  sesionId: selectedSesion.id, // corregido
  activityId: activity.id,
  userId: currentUser.id, // asegúrate de tenerlo disponible
  result: 0.0, // puedes convertir `score` a un número si es necesario
  completedAt: endTime,
  durationSeconds: Math.floor((new Date(endTime).getTime() - new Date(startTime).getTime()) / 1000)
};
            console.log(" Payload enviado a saveSessionActivityResult:",{
                payload
            }) 
            try {
