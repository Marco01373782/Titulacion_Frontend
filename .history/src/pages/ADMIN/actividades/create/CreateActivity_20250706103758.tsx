...
const CreateActivity = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalUrl = activityType === 'ATENCION' ? manualUrl : (useManualUrl ? manualUrl : resourceUrl);

    if (!title || !description || (activityType !== 'ATENCION' && !finalUrl) || !difficulty || !activityType) {
      setToast({ message: 'Completa todos los campos', type: 'error' });
      return;
    }

    if (activityType === 'ATENCION' && questions.length === 0) {
      setToast({ message: 'Agrega al menos una pregunta para actividades de atención', type: 'error' });
      return;
    }

    setLoading(true);
    try {
      const res = await ApiService.post('/activities', {
        title,
        description,
      ...
      if (activityType === 'ATENCION') {
        for (const q of questions) {
          await createQuestion(activityId, q);
        }
      }
  ...
  <FormControl fullWidth margin="normal">
    ...
    >
      {difficulties.map(diff => (
        ...
        {activityType === 'ATENCION' && (
          <Box mt={3}>
            <Typography variant="h6" mb={1}>Agregar pregunta</Typography>
            ...
            {answers.map((ans, idx) => (
              <Box key={idx} display="flex" alignItems="center" gap={1} mb={1}>
                ...
                />
                <FormControlLabel
                  control={<Checkbox checked={correctIndex === idx} onChange={() => setCorrectIndex(idx)} />}
                  label="Correcta"
                />
              </Box>
            ))}
            ...
            <Button
              ...
              onClick={() => {
                if (!currentQuestion || answers.some(a => a.trim() === '')) {
                  setToast({ message: 'Completa la pregunta y todas las respuestas', type: 'error' });
                  return;
                }
            ...
            {questions.length > 0 && (
              <Typography mt={2}>Preguntas agregadas: {questions.length}</Typography>
            )}
          </Box>
        )}
