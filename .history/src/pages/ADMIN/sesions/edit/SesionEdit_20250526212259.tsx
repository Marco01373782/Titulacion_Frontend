import React, { useEffect, useState } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import ApiServic
import './SesionEdit.css';

const SesionEdit = () => {
  const { id } = useParams();
  const navigate = useNavigate();

  const [titulo, setTitulo] = useState('');
  const [descripcion, setDescripcion] = useState('');
  const [dificultadNombre, setDificultadNombre] = useState('');
  const [actividades, setActividades] = useState([]);

  useEffect(() => {
    const fetchSesion = async () => {
      try {
        const sesion = await ApiService.get(`/sesiones/${id}`);
        setTitulo(sesion.titulo);
        setDescripcion(sesion.descripcion);
        setDificultadNombre(sesion.difficulty?.nombre || 'Sin dificultad');

        const actividadesResp = await ApiService.get(`/session-activities/by-session/${id}`);
        const actividadesSolo = actividadesResp.map((sa: any) => sa.activity);
        setActividades(actividadesSolo);
      } catch (error) {
        console.error('Error al cargar la sesión:', error);
      }
    };

    fetchSesion();
  }, [id]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    try {
      await ApiService.put(`/sesiones/${id}`, {
        titulo,
        descripcion,
      });

      alert('Sesión actualizada correctamente');
      navigate('/app/gestionar-sesiones');
    } catch (error) {
      console.error('Error al actualizar la sesión:', error);
      alert('Hubo un error al actualizar la sesión');
    }
  };

  return (
    <div className="sesion-edit-container">
      <h2>Editar Sesión</h2>
      <form onSubmit={handleSubmit} className="sesion-edit-form">
        <label>
          Título:
          <input
            type="text"
            value={titulo}
            onChange={(e) => setTitulo(e.target.value)}
            required
          />
        </label>

        <label>
          Descripción:
          <textarea
            value={descripcion}
            onChange={(e) => setDescripcion(e.target.value)}
            rows={4}
          />
        </label>

        <div className="readonly-field">
          <strong>Dificultad:</strong> {dificultadNombre}
        </div>

        <div className="actividades-lista">
          <h4>Actividades asignadas:</h4>
          {actividades.map((actividad: any) => (
            <div key={actividad.id} className="actividad-card">
              <p><strong>{actividad.nombre}</strong></p>
              <p>Tipo: {actividad.type?.nombre}</p>
              <p>Dificultad: {actividad.difficulty?.nombre}</p>
            </div>
          ))}
        </div>

        <button type="submit" className="btn-guardar">Guardar cambios</button>
      </form>
    </div>
  );
};

export default SesionEdit;
