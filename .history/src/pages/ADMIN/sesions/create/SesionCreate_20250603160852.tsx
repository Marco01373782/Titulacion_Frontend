      ...
      {showManualSelect && sesionId && (
        <ActivitySelector
          ...
          onConfirm={(selectedIds) => {
            ...
            const seleccionadas = filteredActivities.filter((a) => selectedIds.includes(a.id));
            const tipos = new Set(seleccionadas.map((a) => a.type.name));
            if (tipos.size < 4) {
              setToast({
                message: 'Debes seleccionar al menos una actividad por cada tipo (4 tipos en total)',
                type: 'error',
              });
              return;
            }
