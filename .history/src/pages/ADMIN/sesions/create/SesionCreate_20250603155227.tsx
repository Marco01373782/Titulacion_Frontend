    ...
    useEffect(() => {
        fetchDifficulties()
        .then((res) => {
            // Mapeo de string[] a Difficulty[]
            const difficultiesMapped: Difficulty[] = res.data.map((name: string, index: number) => ({
            id: index + 1,
            name,
            }));
            setDifficulties(difficultiesMapped);
        })
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Buscar el nombre de la dificultad seleccionada para enviarla como string
        const difficultyName = difficulties.find((d) => d.id === difficultyId)?.name;

        if (!difficultyName) {
        setToast({ message: 'Dificultad inválida', type: 'error' });
        setLoading(false);
        return;
        }
        ...
        title,
        description,
        difficulty: difficultyName, // Enviamos solo el nombre (string)
        };
    ...
    return (
        ...
        {showManualSelect && sesionId && (
            ...
            selectedActivityIds={[]} // ya no se usa estado aquí
