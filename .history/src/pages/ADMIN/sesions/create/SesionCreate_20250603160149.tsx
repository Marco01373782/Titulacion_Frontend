    ...
    import { Activity } from '../../../../types/ActivityTypes'; // ajusta si la ruta cambia
    ...
    title: string;
    type: { name: string };
    difficulty: string; // ahora string, no objeto con id
    }

    const SesionCreate: React.FC = () => {
    const [title, setTitle] = useState('');
    const [description, setDescription] = useState('');
    const [difficulty, setDifficulty] = useState(''); // string en lugar de id
    const [difficulties, setDifficulties] = useState<string[]>([]); // ahora string[]
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        title,
        description,
        difficulty, // string literal enviado directamente
        };
    ...
    // Filtrar actividades según string difficulty
    const filteredActivities = activities.filter((a) => a.difficulty === difficulty);
    ...
    return (
        ...
        {showManualSelect && sesionId && (
            ...
            selectedActivityIds={[]} // sigue sin estado aquí
            ...
            onConfirm={(selectedIds) => {
                ...
                const selectedActivities = filteredActivities.filter((a) => selectedIds.includes(a.id));
                const tiposSeleccionados = new Set(selectedActivities.map((a) => a.type.name));

                if (tiposSeleccionados.size < 4) {
                setToast({
                    message: 'Debes seleccionar al menos una actividad por cada tipo (hay 4 tipos)',
                    type: 'error',
                });
                return;
                }
