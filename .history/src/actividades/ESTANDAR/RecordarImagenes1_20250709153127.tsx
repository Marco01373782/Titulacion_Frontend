...
const RecordarImagenes1 = ({ onFinish }: { onFinish?: (result: string) => void }) => {
    ...
    const renderGrid = (imagenes: string[]) => (
        <Box
            sx={{
                display: 'grid',
                gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(3, 2fr)',
                gap: 2,
                width: '100%',
                maxWidth: '100%',
                justifyItems: 'center',
        ...
        >
            ...
            {fase === 'seleccionar' && (
                <>
                    ...
                    <Button variant="contained" onClick={evaluar} sx={{ mt: 1 }}>
                        Enviar respuestas
                    </Button>
                </>
            )}
...
export default RecordarImagenes1;
