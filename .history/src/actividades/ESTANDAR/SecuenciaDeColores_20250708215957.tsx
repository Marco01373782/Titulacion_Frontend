...
const ColoresSecuancia = ({ onFinish }: { onFinish?: (result: string) => void }) => {
                    ...
                    <>
                        ...
                        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', justifyContent: 'center', mt: 2 }}>
                            {secuencia.map((color, i) => (
                                <Paper key={i} elevation={3} sx={{ width: isMobile ? 80 : 120, height: isMobile ? 100 : 160, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: isMobile ? '2.5rem' : '4rem' }}>
                                    {color}
                                </Paper>
                            ))}
                        </Box>
