...
const NumeroFaltante = ({ onFinish }: { onFinish?: (result: string) => void }) => {
    ...
    return (
        <Box sx={{ width: '100%', height: '100%', p: 2, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1, background:'red' }}>
            ...
            {fase === 'mostrar' && (
                <>
                    <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', justifyContent: 'center' }}>
                        {numeros.map((num, i) => (
                            <Paper key={i} elevation={4} sx={{ width: 80, height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem' }}>
                                {num}
                            </Paper>
                        ))}
                    </Box>
