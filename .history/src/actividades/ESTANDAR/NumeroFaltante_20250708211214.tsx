...
const NumeroFaltante = ({ onFinish }: { onFinish?: (result: string) => void }) => {
    ...
    return (
        <Box sx={{ width: '100%', height: '100%', p: 3, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 5, background:'red' }}>
            ...
            {fase === 'mostrar' && (
                <>
                    <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap', justifyContent: 'center' ,background:'blue',width: '100%', height: '50%',  display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', }}>
                        {numeros.map((num, i) => (
                            <Paper key={i} elevation={4} sx={{ width: 80, height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem' ,background:'green' }}>
                                {num}
                            </Paper>
                        ))}
                    </Box>
