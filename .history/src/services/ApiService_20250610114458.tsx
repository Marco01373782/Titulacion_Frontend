...
// ✅ ApiService.ts
export const getActivitiesBySession = async (sessionId: number) => {
  try {
    console.log("📦 [getActivitiesBySession] Solicitando actividades para sesión ID:", sessionId);
    const response = await ApiService.get(`/session-activities/by-session/${sessionId}`);
    console.log("✅ [getActivitiesBySession] Actividades obtenidas:", response.data);
    return response.data;
  } catch (error) {
    console.error("❌ [getActivitiesBySession] Error al obtener actividades:", error);
    throw error;
  }
};
