...
const ACTIVITY_TYPE_LABELS: Record<string, string> = {
  ...
  ATENCION: 'Atención',
  LENGUAJE: 'Lenguaje',
  PERCEPCION_VISUAL: 'Percepción Visual',
  ORIENTACION: 'Orientación',
  COORDINACION: 'Coordinación',
  // Agrega todos los enums que manejes
};
...
export const fetchActivityTypes = async (): Promise<AxiosResponse<ActivityTypeDTO[]>> => {
  const res = await ApiService.get<string[]>('/enums/activity-types');
  const formatted = res.data.map(name => ({
    name,
    label: ACTIVITY_TYPE_LABELS[name] || name, // Fallback por si se olvida uno
  }));
  // Retornamos el objeto como si fuera una respuesta Axios normal (para compatibilidad)
  return {
    ...res,
    data: formatted,
  };
